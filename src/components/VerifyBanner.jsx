...
export default function VerifyBanner() {
  ...
  async function resend() {
    ...
    if (r.ok) setSent("Verification email sent. (Dev: see server console.)");
  }

  return (
    <div style={{ background: "#fff3cd", borderBottom: "1px solid #ffeeba", color: "#856404", padding: "8px 16px" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <button className="btn btn-sm" onClick={resend}>Resend</button>
        <Link className="btn btn-sm" to={`/verify-sent?email=${encodeURIComponent(user.email)}`}>Help</Link>
        {sent && <span style={{ marginLeft: 8, fontSize: 12 }}>{sent}</span>}
      </div>
    </div>
  );
}
